...
const ProgressiveIdeaBuilder: React.FC<ProgressiveIdeaBuilderProps> = ({
  ...
  // Generate smart questions based on initial idea
  const generateQuestions = async () => {
    ...
    try {
      ...
      // Generate questions for missing or weak modules
      const moduleQuestions = [
        {
          moduleType: 'problem',
          question: currentLanguage === 'ko' 
            ? `"${initialIdea}"가 해결하려는 핵심 문제는 무엇인가요?`
            : `What is the core problem that "${initialIdea}" aims to solve?`,
        ...
        },
        {
          ...
          question: currentLanguage === 'ko'
            ? '주요 타겟 고객층은 누구인가요?'
            : 'Who is your main target customer segment?',
          suggestedAnswers: currentLanguage === 'ko'
            ? ['직장인', '학생', '주부', '창업가', '시니어']
        ...
        },
        {
          ...
          question: currentLanguage === 'ko'
            ? '고객에게 제공하는 핵심 가치는 무엇인가요?'
            : 'What is the core value you provide to customers?',
          suggestedAnswers: currentLanguage === 'ko'
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">{text[currentLanguage].subtitle}</p>
            </div>
